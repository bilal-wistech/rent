...
$("input[name=phone]").on("blur keyup", function (e) {
    ...
    if ($.trim($(this).val()) !== "") {
        if (
            ...
            var pluginCarrierCode = $("#phone").intlTelInput(
...
function updateControls(addressComponents) {
    ...
    if (
        typeof addressComponents.city !== "undefined" &&
        addressComponents.country !== "undefined" &&
        typeof addressComponents.city !== null &&
        addressComponents.country !== null &&
        typeof addressComponents.city !== "" &&
        addressComponents.country !== ""
    ) {
        $("#map_address").val(
            addressComponents.city + "," + addressComponents.country_fullname
        );
    } else {
        if (
            addressComponents.stateOrProvince != "" &&
            addressComponents.country_fullname != ""
        ) {
            $("#map_address").val(
                addressComponents.stateOrProvince +
                    "," +
...
$("#us3").locationpicker({
    ...
    oninitialized: function (component) {
        var addressComponents =
            $(component).locationpicker("map").location.addressComponents;
        updateControls(addressComponents);
    },
});
...
$("#customer_form").on("submit", function (e) {
    ...
    if (first_name && last_name && email) {
        $.ajax({
            ...
            .done(function (response) {
                if (response.status == 1) {
                    ...
                    // Append new customer to the dropdown
                    $("#user_id").append(
                        '<option data-icon-class="icon-star-alt" value="' +
                            response.user.id +
                            '" selected="selected">' +
        ...
        // Trigger form validation if validation fails
