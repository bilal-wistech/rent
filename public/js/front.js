...
if (expireTime != '') {
  function expirationTimeSet() {
    ...
    if (time_remaining < 0)
      ...
      var h = (Math.floor(time_remaining / _day), Math.floor(time_remaining % _day / _hour)),
        m = Math.floor(time_remaining % _hour / _minute),
        s = Math.floor(time_remaining % _minute / _second);
...
$(document.body).on('click', '.date-package-modal', function(){
    $('#model-message').html("");
        ...
        var dateFormat = front_date_format_type;
