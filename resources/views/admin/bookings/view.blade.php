                         ...
                         <div class="box-body">
                             <div class="box-header">
                                 <div class="pull-right"><a class="btn btn-success f-14"
                                         href="{{ route('admin.bookings.create') }}">Add Bookings</a></div>
                             </div>
                             <form class="form-horizontal" enctype='multipart/form-data'
                                 action="{{ url('admin/bookings') }}" method="GET" accept-charset="UTF-8">
                                 {{ csrf_field() }}
                                     ...
                                     <div class="col-xl-2 col-sm-6 col-12">
                                         ...
                                         <select class="form-control" name="status" id="status">
                                             ...
                                             <option value="Accepted"
                                                 {{ $allstatus == 'Accepted' ? ' selected = "selected"' : '' }}>Accepted
                                             </option>
                                             <option value="Cancelled"
                                                 {{ $allstatus == 'Cancelled' ? ' selected = "selected"' : '' }}>Cancelled
                                             </option>
                                             <option value="Declined"
                                                 {{ $allstatus == 'Declined' ? ' selected = "selected"' : '' }}>Declined
                                             </option>
                                             <option value="Expired"
                                                 {{ $allstatus == 'Expired' ? ' selected = "selected"' : '' }}>Expired
                                             </option>
                                             <option value="Pending"
                                                 {{ $allstatus == 'Pending' ? ' selected = "selected"' : '' }}>Pending
                                             </option>
                                             <option value="Processing"
                                                 {{ $allstatus == 'Processing' ? ' selected = "selected"' : '' }}>
                                                 Processing</option>
                                         </select>
                                     </div>
