                                            ...
                                            @foreach ($propertyPricing as $list)
                                                <div class="mt-2 mb-2 pricing-item">
                                                    ...
                                                    <input type="text" name="prices[]" class="form-control mt-1"
                                                        value="{{ $list->price }}" placeholder="Enter price">
                                                </div>
                                            @endforeach
                                        </div>
    ...
    $(document).ready(function() {
        ...
        function getBasePrice() {
            ...
            $('select[name="pricing_type[]"]').each(function() {
                ...
                const priceField = $(this).closest('.pricing-item').find('input[name="prices[]"]');
                const priceValue = parseFloat(priceField.val());
        ...
        function updatePriceInput(selectElement) {
            const selectedType = $(selectElement).find("option:selected").text();
            const inputField = $(selectElement).closest('.pricing-item').find('input[name="prices[]"]');
            ...
            if (basePrice !== null && baseType !== null) {
                const newPrice = (basePrice / conversionRates[baseType]) * conversionRates[selectedType];
                inputField.val(newPrice.toFixed(2));
            } else {
                inputField.val('');
            }
        }
