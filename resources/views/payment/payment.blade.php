...
@section('main')
                    ...
                    <div class="row justify-content-center">
                        ...
                        @if ($booking_type == 'instant' || $status == 'Processing')
                            ...
                            <div class="col-sm-12 p-0 pb-3">
                                <select name="payment_country" id="country-select" data-saving="basics1"
                                    class="form-control mb20">
                                    @foreach ($country as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ $key == $default_country ? 'selected' : '' }}> {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-sm-12 p-0 pb-3">
                                <select name="payment_method" class="form-control mb20" id="payment-method-select">
                                    ...
                                    @foreach ($gateways as $gateway)
                                        <option value="{{ $gateway->alias }}" data-payment-type="payment-method"
                        ...
                        <div class="col-sm-12 p-0 pb-3">
                            <textarea name="message_to_host" placeholder="{{ __('Type message to Host.') }}" class="form-control mb20"
                ...
                <div class="card p-3">
                    ...
                    <div class="card-body p-2">
                        ...
                        <div class="border p-4 mt-4 text-center rounded-3">
                            ...
                            <div class="text-16"><strong>{{ date(setDateForFront(), strtotime($checkin)) }}</strong> to
                                <strong>{{ date(setDateForFront(), strtotime($checkout)) }}</strong>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        <p class="exfont text-16">
                            ...
                            {!! __("The exchange rate for booking this listing is :x :y to :z :w ( your host's listing currency ).", [
                                'x' => moneyFormat($currentCurrency->symbol, 1),
                                'y' => $currentCurrency->code,
                                'z' => moneyFormat($result->property_price->currency->org_symbol, $price_rate),
                                'w' => $result->property_price->currency_code,
                            ]) !!}
                        </p>
                    </div>
                </div>
    ...
    @push('scripts')
        ...
        <script type="text/javascript" src="{{ asset('js/payment.min.js') }}"></script>
    @endpush
@stop
