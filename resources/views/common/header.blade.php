                ...
                <div class="collapse navbar-collapse offset" id="navbarSupportedContent">
                    <div class="nav navbar-nav menu_nav justify-content-end">
                        @if (Request::segment(1) != 'help')
                            <div class="nav-item">
                                <a class="nav-link p-2" href="{{ url('property/create') }}"
                        ...
                        @if (!Auth::check())
                            <div class="nav-item">
                                <a class="nav-link strip-left" href="{{ url('signup') }}"
                                    aria-label="signup">{{ __('Sign Up') }}</a>
                            </div>
                            <div class="nav-item">
                                <a class="nav-link" href="{{ url('login') }}"
                                    aria-label="login">{{ __('Log In') }}</a>
                            </div>
                        @else
                            <div class="d-flex">
                                <div>
                                ...
                                <div>
                                    <div class="nav-item ml-0 pl-0">
                                        <div class="dropdown">
                                            <a href="javascript:void(0)" class="nav-link dropdown-toggle text-15"
                                                type="button" id="dropdownMenuButton" data-toggle="dropdown"
                                                aria-label="user-profile" aria-haspopup="true" aria-expanded="false">
                                            ...
                                            <div class="dropdown-menu drop-down-menu-left p-0 drop-width text-14"
                                                ...
                                                <a class="vbg-default-hover border-0  font-weight-700 list-group-item vbg-default-hover border-0"
                                                    href="{{ url('dashboard') }}"
                                                    aria-label="dashboard">{{ __('Dashboard') }}</a>
                                                <a class="font-weight-700 list-group-item vbg-default-hover border-0 "
                                                    href="{{ url('users/profile') }}"
                                                    aria-label="profile">{{ __('Profile') }}</a>
                                                <a class="font-weight-700 list-group-item vbg-default-hover border-0 "
                                                    href="{{ url('logout') }}"
                                                    aria-label="logout">{{ __('Logout') }}</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <ul class="mobile-side">
                    @if (Auth::check())
                        <li><a href="{{ url('dashboard') }}"><i
                                    class="fa fa-tachometer-alt mr-3"></i>{{ __('Dashboard') }}</a></li>
                        <li><a href="{{ url('inbox') }}" class="d-flex justify-content-between align-items-center">
                                <div><i class="fas fa-inbox mr-3"></i>{{ __('Inbox') }}</div>
                                @php
                                    $count = getInboxUnreadCount();
                                @endphp
                                @if ($count)
                                    <span
                                        class="badge badge-danger rounded-circle mr-2 text-12">{{ $count }}</span>
                                @endif
                            </a></li>
                        <li><a href="{{ url('properties') }}"><i
                                    class="far fa-list-alt mr-3"></i>{{ __('Listings') }}</a></li>
                        <li><a href="{{ url('my-bookings') }}"><i
                                    class="fa fa-bookmark mr-3"></i>{{ __('Bookings') }}</a></li>
                        ...
                        <li><a href="{{ url('users/transaction-history') }}"><i
                                    class="fas fa-money-check-alt mr-3 text-14"></i> {{ __('Transactions') }}</a></li>
                        <li><a href="{{ url('users/profile') }}"><i
                                    class="far fa-user-circle mr-3"></i>{{ __('Profile') }}</a></li>
                        ...
                        <div class="collapse" id="collapseExample">
                            <ul class="ml-4">
                                <li><a href="{{ url('users/reviews') }}"
                                        class="text-14">{{ __('Review About You') }}</a></li>
                                <li><a href="{{ url('users/reviews_by_you') }}"
                                        class="text-14">{{ __('Review By You') }}</a></li>
                            </ul>
                        </div>
                        <li><a href="{{ url('logout') }}"><i
                                    class="fas fa-sign-out-alt mr-3"></i>{{ __('Logout') }}</a></li>
                    @else
                        <li><a href="{{ url('signup') }}"><i class="fas fa-stream mr-3"></i>{{ __('Sign Up') }}</a>
                        </li>
                        <li><a href="{{ url('login') }}"><i
                                    class="far fa-list-alt mr-3"></i>{{ __('Log In') }}</a></li>
                    @endif
