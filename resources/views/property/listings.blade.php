                                        ...
                                        <div class="d-flex">
                                            ...
                                            <div>
                                                <form action="{{ url('/properties') }}" method="POST" id="listing-form">
                                                    {{ csrf_field() }}
                                                    <select class="form-control text-center text-14 minus-mt-6"
                                                        id="listing_select" name="status">
                                                        <option value="All"
                                                            {{ ($status ?? '') == 'All' ? ' selected = "selected"' : '' }}>
                                                            {{ __('All') }}</option>
                                                        <option value="Listed"
                                                            {{ ($status ?? '') == 'Listed' ? ' selected = "selected"' : '' }}>
                                                            {{ __('Listed') }}</option>
                                                        <option value="Unlisted"
                                                            {{ ($status ?? '') == 'Unlisted' ? ' selected = "selected"' : '' }}>
                                                            {{ __('Unlisted') }}</option>
                                                    </select>
                                                </form>
                                            </div>
                                ...
                                <div class="col-md-12 p-0 mb-4">
                                    <div class=" row  border p-2 rounded-3">
                                            ...
                                            <div class="img-event">
                                                <a href="properties/{{ $property->slug }}">
                                                    <img class="room-image-container200 rounded"
                                                        src="{{ $property->cover_photo }}" alt="cover_photo">
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6 col-xl-6 p-2">
                                            <div>
                                                <a href="properties/{{ $property->slug }}">
                                                    <p class="mb-0 text-18 text-color font-weight-700 text-color-hover">
                                            ...
                                            <div class="review-0 mt-4">
                                                <div class="d-flex justify-content-between">
                                                    ...
                                                    <div class="pr-5">
                                                        <span class="font-weight-700 text-20">{!! moneyFormat($currentCurrency->symbol, optional($property->property_price)->price) !!}</span>
                                                        / {{ __('night') }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <ul class="list-inline mt-2 pb-3">
                                                <li class="list-inline-item border rounded-3 p-1 mt-4 pl-3 pr-3">
                                                    <p class="text-center mb-0">
                                                        <i
                                                            class="fas fa-bed text-20 d-none d-sm-inline-block text-muted"></i>
                                                        {{ $property->bedrooms }}
                                                ...
                                                <li class="list-inline-item  border rounded-3 mt-4 p-1  pl-3 pr-3">
                                                    <p class="text-center mb-0">
                                                        <i
                                                            class="fas fa-user-friends d-none d-sm-inline-block text-20 text-muted"></i>
                                                ...
                                                <li class="list-inline-item  border rounded-3 mt-4 p-1  pl-3 pr-3">
                                                    <p class="text-center mb-0">
                                                        <i
                                                            class="fas fa-bath text-20  d-none d-sm-inline-block  text-muted"></i>
                                                        {{ $property->bathrooms }}
                                                ...
                                                <div class="align-self-center w-100">
                                                    <div class="row">
                                                            ...
                                                            <div class="main-toggle-switch text-left text-sm-center">
                                                                @if ($property->steps_completed == 0)
                                                                    @if ($property_approval == 'Yes' && $property->is_verified == 'Pending')
                                                                        <span
                                                                            class="badge badge-warning p-3 pl-4 pr-4 text-14 border-r-25">{{ __('Waiting for verify') }}</span>
                                                                    @else
                                                                        <label class="toggleSwitch large" onclick="">
                                                                            <input type="checkbox" id="status"
                                                                                data-id="{{ $property->id }}"
                                                                                data-status="{{ $property->status }}"
                                                                                {{ $property->status == 'Listed' ? 'checked' : '' }} />
                                                                            <span>
                                                                ...
                                                                @else
                                                                    <span
                                                                        class="badge badge-warning p-3 pl-4 pr-4 text-14 border-r-25">{{ $property->steps_completed }}
                                                                        {{ __('steps to listed') }}</span>
                                                                @endif
                                                            </div>
                                                        </div>
                                                        ...
                                                        @if ($property->steps_completed != 0 && $property->is_verified == 'Pending' && $property_approval == 'Yes')
                                                            <div class="col-6 col-sm-12">
                                                                <a
                                                                    href="{{ url('listing/' . $property->id . '/basics') }}">
                                                                    <div
                                                                        class="text-color text-color-hover text-14 text-right text-sm-center mt-0 mt-md-3 p-2">
                            ...
                            @empty
                                <div class="row jutify-content-center position-center w-100 p-4 mt-4">
                                    <div class="text-center w-100">
                                        <img src="{{ asset('img/unnamed.png') }}" class="img-fluid" alt="Not Found">
                                        <p class="text-center">
                                            {{ __('You don’t have any Listing yet—but when you do, you’ll find them here.') }}
                                        </p>
                                    </div>
                                </div>
                            @endforelse
